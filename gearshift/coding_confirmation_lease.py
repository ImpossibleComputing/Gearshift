"""Confirmation-stage independent RunPod lease guard with a dollar-only ceiling.
...
def verify_lease(lease):
    ...
    for field in ('allocation_epoch', 'deadline_epoch', 'upper_hourly_usd',
                  'baseline_usd', 'total_cap_usd', 'cleanup_reserve_usd'):
        value = lease.get(field)
        if type(value) not in (int, float) or not math.isfinite(value) or value < 0:
            raise ValueError(f'invalid {field}')
    if lease['total_cap_usd'] != 2500 or lease.get('total_cap_gpu_hours') is not None:
        raise ValueError('confirmation authority is hard2500 with no GPU-hour ceiling')
    if lease['cleanup_reserve_usd'] != 40:
        raise ValueError('lease must reserve forty dollars for cleanup')
    if type(lease.get('gpu_count')) is not int or not 0 <= lease['gpu_count'] <= 8:
    ...
    gpu_hours = (lease.get('baseline_gpu_hours', 0) + hours * lease['gpu_count']
                 + lease.get('other_reserved_gpu_hours', 0))
    if reserved >= lease['total_cap_usd']:
    ...
    return {'reserved_total_usd': reserved, 'reserved_total_gpu_hours': gpu_hours,
            'allocation_hours': hours, 'soft_target_exceeded': reserved > 2000}
