#!/usr/bin/env python3
"""One-hour read-only retrieval of the declared private scoring input on retained storage."""
import argparse
import fcntl
import json
...
def allocate(name):
    if not r.re.fullmatch('private_inputs_reader_ap[0-9]{2}',name):raise ValueError('Expected unique private_inputs_reader_apNN name')
    with (b.EVIDENCE/'allocation.lock').open('a') as lock:
        ...
        lease={'experiment_id':b.EXPERIMENT,'pod_id':'reservation_pending','allocation_epoch':now,
            ...
            'purpose':'frozen_private_input_retrieval_no_generation'}
