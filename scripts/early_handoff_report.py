...
def main():
 ...
 fig,axes=plt.subplots(1,3,figsize=(14,4));q=[summary['conditions'][c]['pass_rate'] for c in order];sec=[summary['conditions'][c]['mean_stage_seconds'] for c in order]
 axes[0].plot(fractions,q,'o-');axes[0].set(xlabel='Source reasoning fraction (%)',ylabel='Mean success across three draws')
 axes[1].plot(fractions,sec,'o-');axes[1].set(xlabel='Source reasoning fraction (%)',ylabel='Single-output stage sum (s)')
 for c,v in summary['conditions'].items():axes[2].scatter(v['mean_stage_seconds'],v['pass_rate']);axes[2].annotate(c,(v['mean_stage_seconds'],v['pass_rate']),fontsize=7)
 axes[2].set(xlabel='Single-output stage sum (s)',ylabel='Mean success');fig.suptitle('Early handoff: development screen, M4 Max BF16; retrospective fractions');fig.tight_layout();fig.savefig(a.output/'frontier.png',dpi=150);plt.close(fig)
 ...
 (ROOT/'EARLY_HANDOFF_RESULTS.md').write_text('\n'.join(lines)+'\n')
