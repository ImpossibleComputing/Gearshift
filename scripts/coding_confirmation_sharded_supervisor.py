#!/usr/bin/env python3
"""New operational supervisor for static task shards; no numerical runtime changes."""
import argparse
import json
import os
...
from scripts.coding_confirmation_generation_supervisor import release as primary_release, validate_guard
...
def release(root, lease, folder, outcome, plan_path):
    plan = read(plan_path)
    for source in [plan_path, *(root / 'sharding').rglob('*.json')]:
    ...
    for relative in sorted(set(paths)):
        source = sharded.primary.scoped_file(ROOT, relative)
        receipt = {'bytes': source.stat().st_size, 'sha256': sha256_file(source)}
        copy_verified(source, folder / 'operational_provenance' / relative, receipt)
    primary_release(root, lease, folder, outcome)
...
def run(plan_path, plan_sha256):
    ...
    plan = read(plan_path)
    if plan.get('execution_mode') != 'generation': raise ValueError('Supervisor requires a generation dispatch')
    ...
    folder = control / 'sharded_generation_supervisor'; folder.mkdir(parents=True)
    ...
    children = {}; attempts = {}; stop = False; ownership_verified = True; outcome = 'ready_sharded_jobs_drained'
    ...
    def start(gpu):
        ...
        command = [str(ROOT / '.pilot-venv/bin/python'), 'scripts/coding_confirmation_sharded_generate.py',
            'run', '--plan', str(plan_path.relative_to(ROOT)), '--plan-sha256', plan_sha256,
            '--worker-id', lease['pod_id'] + '_generation_' + str(gpu), '--preference', plan['preferences'][gpu]]
    ...
    try:
        public = sharded.public_context(ROOT, str(plan_path.relative_to(ROOT)), plan_sha256)
        sharded.validate_generation_store(public, verify_local=True)
        if len(plan['preferences']) != lease['gpu_count'] or any(p not in ('source', 'small', 'receiver') for p in plan['preferences']):
            raise ValueError('Sharded preferences differ from allocation')
