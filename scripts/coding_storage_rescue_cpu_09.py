...
C=ROOT/'evidence/coding_pilot_v1/control';RUN='storage_inspect_cpu_09';D=C/'storage_inspection_cpu_09';D.mkdir(exist_ok=False)
IMAGE='runpod/pytorch@sha256:0a360022e8de4375af99430f84e8b38951acc397252163a37ceac7204d01be35';VOLUME='jj2zyi9yrc';name=PREFIX+RUN+'-worker';started=time.time();deadline=started+3600
...
try:
 ...
 volumes=[v for v in cli('network-volume','list') if v['id']==VOLUME];assert len(volumes)==1 and volumes[0]['dataCenterId']=='US-GA-2'
 pod=cli('pod','create','--name',name,'--image',IMAGE,'--compute-type','CPU','--cloud-type','SECURE','--data-center-ids','US-GA-2','--container-disk-in-gb','20','--network-volume-id',VOLUME,'--ports','22/tcp','--ssh')
 ...
 code="import pathlib,json,time; r=pathlib.Path('/workspace/Gearshift'); w=r/'evidence/coding_pilot_v1/parallel/parallel_day_20260916-memory/memory'; out={}; out['epoch']=time.time(); out['worker_files']=[p.name for p in w.iterdir()]; out['statuses']={p.name:p.read_text() for p in w.glob('*status.json')}; out['bootstrap_log_tail']=(w/'bootstrap.log').read_text()[-12000:]; out['result_roots']=[str(p.relative_to(r)) for p in (r/'results/coding_pilot_v1/parallel_day_20260916-memory').glob('*')]; print(json.dumps(out))"
