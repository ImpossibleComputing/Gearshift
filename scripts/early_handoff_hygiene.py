...
def forbidden(value):
 ...
 return False
def main():
 p=argparse.ArgumentParser();p.add_argument('--raw',type=Path,default=ROOT/'data/coding_pilot_v1/raw');a=p.parse_args()
 ...
 blobs={n:subprocess.check_output(['git','show',':'+n],cwd=ROOT) for n in names};issues=[];by_window={}
 ...
 diff=subprocess.check_output(['git','diff','--cached','--binary'],cwd=ROOT)
 ...
 print(json.dumps({'staged_paths':names,'official_rows_scanned':seen,'issues':sorted(set(issues)),'gitleaks_exit':leak.returncode,'staged_diff_sha256':hashlib.sha256(diff).hexdigest()},indent=2))
