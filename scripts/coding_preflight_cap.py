...
ROOT=Path(__file__).resolve().parents[1]; E=ROOT/'evidence/coding_pilot_v1'; R=ROOT/'results/coding_pilot_v1/preflight_cap_v1'
...
def main():
    ...
    allocation=json.loads((E/'allocation_cap_v1.json').read_text())
    ...
    parent_root=ROOT/'results/coding_pilot_v1/preflight_v5'
    from gearshift.coding_cap_amendment import validate_parent,replay_constraints,NEW_CAP
    checkpoint=validate_parent(parent_root,ROOT)
    declaration=ROOT/'configs/coding_pilot_v1/cap_amendment_v1.json'
    if allocation['cap_amendment_sha256']!=sha(declaration):raise ValueError('Cap amendment declaration differs')
    ...
    ident={'config_sha256':sha(ROOT/'configs/coding_pilot_v1/pilot.json'),'data_identity':json.loads((ROOT/'data/coding_pilot_v1/identity.json').read_text()),
        ...
        'allocation_sha256':sha(E/'allocation_cap_v1.json')}
    ...
    repeat_ids=set(checkpoint['repeat_task_ids'])
    parent_progress=json.loads((parent_root/'progress.json').read_text())
    parent_average=parent_progress['observed_baseline_seconds_including_parent']/40
    def save_progress():
        ...
        summary={'completed_tasks':len(completed),'total_tasks':40,
            ...
            'reasoning_cap_both_models':NEW_CAP,'amendment_number':1}
    ...
    for index,task in enumerate(tasks):
        ...
        previous,previous_complete,lineage=replay_constraints(parent_root,parent,tid)
