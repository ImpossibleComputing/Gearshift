...
from coding_parallel_session import remote,verify_created_pod,controller_heartbeat
from gearshift.coding_control import PREFIX,write,sha,decision
C=ROOT/'evidence/coding_pilot_v1/control';RUN='storage_inspect_05';D=C/'storage_inspection_05';D.mkdir(exist_ok=False)
...
write(D/'intent.json',{'epoch':started,'deadline_epoch':deadline,'purpose':'Inspect retained interrupted worker storage and verify evidence; no model inference or changed task draws','volume_id':VOLUME,'controller_id':RUN,'maximum_gpu_hours':1,'maximum_upper_usd':5.56})
write(C/'allocation_intents'/f'{RUN}.json',{'controller_id':RUN,'started_epoch':started,'resource_names':[name],'gpu_type':'NVIDIA H200','gpu_count':1})
...
try:
 tick();d=decision(json.loads((C/'ledger.json').read_text()));assert not d['stop'] and d['upper_usd']+5.56<980
 ...
 pod=cli('pod','create','--name',name,'--image',IMAGE,'--gpu-id','NVIDIA H200','--gpu-count','1','--cloud-type','SECURE','--data-center-ids','US-CA-2','--container-disk-in-gb','40','--network-volume-id',VOLUME,'--ports','22/tcp','--ssh')
 write(C/'resource_receipts'/f"{pod['id']}.json",{'kind':'pod','id':pod['id'],'name':pod['name'],'started_epoch':time.time(),'upper_rate_usd':5.52,'controller_id':RUN,'gpu_count':1,'gpu_type':'NVIDIA H200'})
 verified=verify_created_pod(pod,VOLUME,IMAGE);write(D/'pod_verified.json',verified)
 conn=None
 for _ in range(60):
  try:
