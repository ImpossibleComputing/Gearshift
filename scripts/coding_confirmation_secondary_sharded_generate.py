...
IMPLEMENTATION={'scripts/coding_confirmation_secondary_sharded_generate.py',
    'scripts/coding_confirmation_secondary_sharded_supervisor.py','scripts/coding_confirmation_secondary_shard_merge.py'}
...
def idle_gpu():
    ...
    # Independently catch an allocated process that has not loaded its weights.
    ...
    if not visible:raise ValueError('Visible GPU ownership is not identifiable')
    for line in rows.splitlines():
        pid,_,uuid=line.partition(',')
        if uuid.strip()==visible and int(pid.strip())!=os.getpid():
            raise ValueError('Assigned secondary GPU has another live compute owner')
...
def work(c):
    ...
    from scripts.coding_confirmation_secondary_shard_merge import seal_shard
    seal_shard(c)
