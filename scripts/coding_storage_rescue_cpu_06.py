...
C=ROOT/'evidence/coding_pilot_v1/control';RUN='storage_inspect_cpu_06';D=C/'storage_inspection_cpu_06';D.mkdir(exist_ok=False)
...
try:
 ...
 pod=cli('pod','create','--name',name,'--image',IMAGE,'--compute-type','CPU','--gpu-count','0','--cloud-type','SECURE','--data-center-ids','US-CA-2','--container-disk-in-gb','20','--network-volume-id',VOLUME,'--ports','22/tcp','--ssh')
 ...
 detail=pod_detail_rest(pod['id']);assert detail['id']==pod['id'] and detail['name']==name and detail['imageName']==IMAGE;assert detail.get('gpuCount')==0 and 0<float(detail.get('costPerHr',0))<=.50;assert (detail.get('networkVolumeId') or (detail.get('networkVolume') or {}).get('id'))==VOLUME;write(D/'pod_verified.json',sanitized_pod(detail))
 conn=None
 for _ in range(60):
  try:
