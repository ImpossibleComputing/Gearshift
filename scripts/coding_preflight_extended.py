...
ROOT=Path(__file__).resolve().parents[1]; E=ROOT/'evidence/coding_pilot_v1'; R=ROOT/'results/coding_pilot_v1/preflight_v4'
...
def main():
    ...
    allocation=json.loads((E/'allocation_extension.json').read_text())
    ...
    parent_root=ROOT/'results/coding_pilot_v1/preflight_v3'
    parent=compatible_parent(parent_root,ROOT)
    ...
    ident={'config_sha256':sha(ROOT/'configs/coding_pilot_v1/pilot.json'),'data_identity':json.loads((ROOT/'data/coding_pilot_v1/identity.json').read_text()),
        ...
        'budget_extension_sha256':sha(E/'preflight_extension_approved.json')}
    ...
    prior_n=parent_progress['completed_tasks']
    prior_elapsed=parent_progress['remaining_baseline_forecast_seconds']*prior_n/(40-prior_n) if prior_n<40 else 0
