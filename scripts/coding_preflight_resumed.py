...
ROOT=Path(__file__).resolve().parents[1]; E=ROOT/'evidence/coding_pilot_v1'; R=ROOT/'results/coding_pilot_v1/preflight_v5'
...
def main():
    ...
    allocation=json.loads((E/'allocation_resume_09.json').read_text())
    ...
    parent_root=ROOT/'results/coding_pilot_v1/preflight_v4'
    from gearshift.coding_checkpoint import validate_checkpoint
    checkpoint=validate_checkpoint(parent_root,ROOT,6)
    parent=checkpoint['parent_identity']
    if allocation['approval_sha256']!=sha(E/'experiment_100h_approved.json'):
        raise ValueError('Allocation authorization identity differs')
    if allocation['parent_checkpoint']!= {k:v for k,v in checkpoint.items() if k!='parent_identity'}:
    ...
    ident={'config_sha256':sha(ROOT/'configs/coding_pilot_v1/pilot.json'),'data_identity':json.loads((ROOT/'data/coding_pilot_v1/identity.json').read_text()),
        ...
        'budget_extension_sha256':sha(E/'experiment_100h_approved.json'),
        'allocation_sha256':sha(E/'allocation_resume_09.json')}
    ...
    prior_n=checkpoint['completed_tasks']
    prior_elapsed=checkpoint['observed_seconds']
    for index,task in enumerate(tasks):
        tid=task['task_id'];folder=R/'tasks'/tid.replace('/','__')
        upstream=parent_root/'tasks'/tid.replace('/','__')
        if (upstream/'complete.json').exists():
            row=reuse_completed(upstream,folder,parent,ident,tid,str(upstream.relative_to(ROOT)))
            completed.append(row)
            if len(completed)==prior_n:
                saved=json.loads((parent_root/'progress.json').read_text())
                saved.update(preflight_remaining_seconds=deadline-time.time(),reused_parent_tasks=prior_n,forecast_exceeds_preflight=checkpoint['remaining_forecast_seconds']>deadline-time.time())
                write(R/'progress.json',saved)
            continue
        ...
        elapsed=prior_elapsed+time.monotonic()-start;forecast=elapsed/len(completed)*(40-len(completed))
        summary={'completed_tasks':len(completed),'total_tasks':40,'passes':{arm:sum(r['pass'][arm] for r in completed) for arm in ['A','B','D']},
            'preflight_remaining_seconds':deadline-time.time(),'observed_baseline_seconds_including_parent':elapsed,'reused_parent_tasks':prior_n,'remaining_baseline_forecast_seconds':forecast,
