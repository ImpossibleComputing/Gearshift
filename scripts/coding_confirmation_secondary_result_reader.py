#!/usr/bin/env python3
"""One-hour read-only export of completed secondary results on retained Colorado storage."""
import argparse
import fcntl
import json
...
def allocate(name):
    if not r.re.fullmatch('secondary_reader_usco[0-9]{2}',name):raise ValueError('Expected unique secondary_reader_uscoNN name')
    with (b.EVIDENCE/'allocation.lock').open('a') as lock:
        ...
        volume=b.api('network-volumes/zeicr9elbn')
        if any(volume.get(k)!=v for k,v in {'id':'zeicr9elbn','size':200,'dataCenter':'US-CO-1','type':'STANDARD'}.items()):
        ...
        lease={'experiment_id':b.EXPERIMENT,'pod_id':'reservation_pending','allocation_epoch':now,
            ...
            'cleanup_reserve_usd':40,'allowed_result_root':'/workspace/GearshiftConfirmationFranceSecondary/'+b.RESULT,
            'purpose':'completed_secondary_export_no_generation'}
        request={'name':'gearshift-confirmation-'+name,'image':b.IMAGE,'disk':40,'cloud':'SECURE',
            'dataCenterIds':['US-CO-1'],'ports':['22/tcp'],'startSsh':True,
            'mounts':{'network':[{'volumeId':'zeicr9elbn','path':'/workspace'}]},
        ...
        b.atomic_json(folder/'allocated_pod_identity.json',{'pod_id':pod['id'],'network_volume_id':'zeicr9elbn'})
        ...
        lease.update(pod_id=pod['id'],network_volume_id='zeicr9elbn',control_relative='allocations/'+pod['id'])
